...
public final class Parser {
    ...
    public func parse() -> AST.Program {
        ...
        while let t = peek {
            ...
            case let .Keyword(keywordKind):
                ...
                case .Infix: statement = parseOperatorDecl(modifiers, attributes)
                case .Prefix: statement = parseOperatorDecl(modifiers, attributes)
                case .Postfix: statement = parseOperatorDecl(modifiers, attributes)
                ...
                default:
                    ...
                    statement = errorStatement(
                        from: token, to: token.sourceRange(in: buffer).end
                    )
                }
    ...
    private func parseBasicStatement(allowImport: Bool = true) -> AST.Statement? {
        ...
        case let .Keyword(keywordKind):
            ...
            case .Infix: return parseOperatorDecl(modifiers, attributes)
            case .Prefix: return parseOperatorDecl(modifiers, attributes)
            case .Postfix: return parseOperatorDecl(modifiers, attributes)
    ...
    private func parseImport() -> AST.Statement {
        ...
        _pathLoop: while let dotToken = peek, case .Operator(.Dot) = dotToken.kind {
            ...
            case .Separator(.OpenBrace):
                index += 2
                let (items, listEnd) = parseImportList(openBrace: afterDot)
                selector = .List(items)
                endToken = listEnd
                break _pathLoop
            default:
                ...
                case .Keyword(.SelfTypeKw):
                    emitError(
                        "'Self' can only appear at the beginning of an import path", at: afterDot
                    )
                    index += 1
                    path.append(afterDot)
                    endToken = afterDot
                case .Keyword(.SelfKw):
                    emitError(
                        "'self' is not allowed in import path, use 'Self' instead", at: afterDot
                    )
                    index += 1
                    path.append(afterDot)
                    endToken = afterDot
        ...
        let node = buildImportNode(path: path, selector: selector, alias: alias)
        return AST.Import(
            token, node,
    ...
    private func buildImportNode(path: [Token], selector: AST.ImportNode?, alias: Token?)
        -> AST.ImportNode
    {
        if let selector {
            var node = selector
            for token in path.reversed() {
                node = .Member(token, node)
            }
            return node
        }
        if path.count == 1 {
        ...
        let last: AST.ImportNode = alias.map { .Alias(terminal, $0) } ?? .Name(terminal)
        var node = last
        for token in path.dropLast().reversed() {
            node = .Member(token, node)
        }
        return node
    }

    private func parseImportList(openBrace: Token) -> ([AST.ImportNode], Token) {
        var items: [AST.ImportNode] = []
        var endToken: Token = openBrace
        while let t = peek {
            if case .Separator(.CloseBrace) = t.kind {
                break
            }
            items.append(parseImportItem())
        ...
        if items.isEmpty {
            emitError("expected import item after '{'", at: openBrace)
        }
        ...
        return (items, endToken)
    }

    private func parseImportItem() -> AST.ImportNode {
        guard let t = peek else {
            return .Name(errorToken())
        }
        let kind: AST.ImportNode
        switch t.kind {
        case .Keyword(.SelfKw):
            index += 1
            kind = .Self_(t)
        ...
        if case let .Name(nameToken) = kind,
           let dotToken = peek, case .Operator(.Dot) = dotToken.kind,
           let afterDot = peek2
        {
            if case .Separator(.OpenBrace) = afterDot.kind {
                index += 2
                let (nested, _) = parseImportList(openBrace: afterDot)
                return .Member(nameToken, .List(nested))
            }
            if case .Operator(.Multiply) = afterDot.kind {
                index += 2
                return .Member(nameToken, .Wildcard(afterDot))
            }
        }
        var alias: Token? = nil
        if let asToken = peek, case .Keyword(.As) = asToken.kind {
        ...
        if let a = alias {
            return applyAlias(kind, a)
        }
        return kind
    }

    private func applyAlias(_ node: AST.ImportNode, _ alias: Token) -> AST.ImportNode {
        switch node {
        case let .Name(token): .Alias(token, alias)
        case let .Self_(token): .Alias(token, alias)
        case let .Member(name, sub): .Member(name, applyAlias(sub, alias))
        case let .Alias(token, _): .Alias(token, alias)
        default: node
        }
    }
    ...
    private func parseOperatorImport(_ importToken: Token) -> AST.Statement {
        ...
        let node = buildImportNode(path: path, selector: selector, alias: nil)
    ...
    private func parseOperatorSelector() -> (AST.ImportNode, Token)? {
        guard let t = peek else {
            emitError(
                "expected operator name, '*', or '{' after '.', but got end of file",
                at: endOfFile
    ...
    private func parseOperatorList(_ openBrace: Token) -> (AST.ImportNode, Token)? {
        var items: [AST.ImportNode] = []
        var endToken: Token = openBrace
    ...
    private func parseOperatorImportItem() -> AST.ImportNode? {
        guard let t = peek else {
        ...
        case .Identifier, .Keyword(.Module):
            ...
            return .Member(moduleToken, result.0)
        default:
            emitError(
    ...
    private func parseOperatorDecl(
        ...
        default:
            ...
            var endLocation = kindToken.sourceRange(in: buffer).end
            if case .Keyword(.Operator) = kindToken.kind {
                if let name = next {
                    endLocation = name.sourceRange(in: buffer).end
                    if let kindWord = peek {
                        switch kindWord.kind {
                        case .Keyword(.Infix), .Keyword(.Prefix), .Keyword(.Postfix):
                            index += 1
                            endLocation = kindWord.sourceRange(in: buffer).end
                        default:
                            break
                        }
                    }
            ...
            return errorStatement(from: kindToken, to: endLocation)
        }
        guard let opKeyword = next else {
            emitError("expected 'operator' after '\(kindToken.value)'", at: endOfFile)
            return errorStatement(from: kindToken, to: endOfFile)
        }
        guard case .Keyword(.Operator) = opKeyword.kind else {
            emitError(
                "expected 'operator' after '\(kindToken.value)', but got '\(opKeyword.value)'",
                at: opKeyword
            )
            return errorStatement(from: kindToken, to: opKeyword)
        }
        guard let name = next else {
        ...
        guard case .Operator = name.kind else {
            ...
            return errorStatement(from: kindToken, to: name)
        }
        ...
        let sourceRange =
            if let endLocation {
                SourceRange(start: kindToken.sourceRange(in: buffer).start, end: endLocation)
            } else {
                SourceRange(from: kindToken, to: name, in: buffer)
            }
        return AST.OperatorDecl(
            modifiers, attributes, opKeyword, name, kind, group,
            sourceRange: sourceRange
        )
    }
        ...
        if case .Separator(.Colon)? = peek?.kind {
            ...
            while let name = peek, case .Identifier = name.kind {
                ...
                guard let eq = peek, case .Operator(.Assign) = eq.kind else {
                    emitError(
                        "expected '=' after asm operand name, but got '\(peek?.value ?? "")'",
                        at: peek ?? token
                    )
                ...
                guard let kind = peek, case .Identifier = kind.kind else {
                    emitError(
                        "expected operand kind after '=', but got '\(peek?.value ?? "")'",
                        at: peek ?? token
                    )
                    skipAsmUntilSyncPoint()
                    break
                ...
                guard let openParen = peek, case .Separator(.OpenParen) = openParen.kind else {
                    emitError(
                        "expected '(' after asm operand kind, but got '\(peek?.value ?? "")'",
                        at: peek ?? token
                    )
                    skipAsmUntilSyncPoint()
                    break
                ...
                guard let constraint = peek, case .Identifier = constraint.kind else {
                    emitError(
                        "expected constraint after '(', but got '\(peek?.value ?? "")'",
                        at: peek ?? token
                    )
                    skipAsmUntilSyncPoint()
                    break
                ...
                guard let closeParen = peek, case .Separator(.CloseParen) = closeParen.kind else {
                    emitError(
                        "expected ')' after asm constraint, but got '\(peek?.value ?? "")'",
                        at: peek ?? token
                    )
                    skipAsmUntilSyncPoint()
                    break
    ...
    private func parseExpression(
        ...
        _loop: while let token = peek {
            ...
            case .Operator(.QuestionMark) where lastIsExpression:
                ...
                index += 1
                lastIsExpression = true
            case .Keyword(.As) where lastIsExpression && castDepth == 0,
