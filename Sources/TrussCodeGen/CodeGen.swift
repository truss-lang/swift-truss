...
public final class CodeGen: TIR.Visitor {
  private static let defaultDataLayout =
  ...
  public func generate(_ mod: TIR.Module) -> LLVMSwiftBinding.Module {
    ...
    llvmModule.setDataLayout(Self.defaultDataLayout)
    if let targetTriple {
      llvmModule.target = LLVMSwiftBinding.TargetMachine.normalizedTriple(targetTriple)
    }
    module = llvmModule
    ...
    for tirFunction in mod.functions {
      functionMap[tirFunction.id] = createFunctionDecl(tirFunction)
    }
    for tirFunction in mod.functions {
  ...
  private func lowerGlobal(_ global: TIR.GlobalVariable) {
    ...
    globalMap[global.id] = llvmGlobal
  }
  ...
  private func payloadByteSize(_ enumType: TIRType.EnumType) -> Int {
    var maximum = 0
    for caseInfo in enumType.cases {
      for associatedType in caseInfo.associatedTypes {
  ...
  private func materialize(_ literal: TIR.BoolLiteral) -> LLVMSwiftBinding.Value? {
    return llvmContext.constantInt(literal.value ? 1 : 0, type: llvmContext.int1)
  }

  private func materialize(_ literal: TIR.NullptrLiteral) -> LLVMSwiftBinding.Value? {
    return llvmContext.constantNull(lowerType(literal.ty))
  }
  ...
  private func aggregateLLVMType(of instruction: TIR.StructElementAddr) -> LLVMSwiftBinding.LLVMType
  {
    lowerType(pointeeTypeID(instruction.base.ty))
  }

  private func aggregateLLVMType(of instruction: TIR.TupleElementAddr) -> LLVMSwiftBinding.LLVMType
  {
    lowerType(pointeeTypeID(instruction.base.ty))
  }
  ...
  public override func visitStructElementAddr(
    ...
    let structType = aggregateLLVMType(of: instruction)
  ...
  public override func visitTupleElementAddr(
    ...
    guard let builder, let base = value(instruction.base) else { return nil }
    let tupleType = aggregateLLVMType(of: instruction)
  ...
  public override func visitUnaryArith(
    ...
    case .Neg:
      built = kind == .Float ? builder.buildFNeg(operand) : builder.buildNeg(operand)
    case .Not:
      built = builder.buildNot(operand)
    case .Bitnot:
      built = builder.buildNot(operand)
    default:
      built = nil
  ...
  public override func visitBinaryArith(
    ...
    switch instruction.op {
    case .Add:
      built = kind == .Float ? builder.buildFAdd(lhs, rhs) : builder.buildAdd(lhs, rhs)
    case .Sub:
      built = kind == .Float ? builder.buildFSub(lhs, rhs) : builder.buildSub(lhs, rhs)
    case .Mul:
      built = kind == .Float ? builder.buildFMul(lhs, rhs) : builder.buildMul(lhs, rhs)
    case .SDiv:
      built = kind == .Float ? builder.buildFDiv(lhs, rhs) : builder.buildSDiv(lhs, rhs)
    case .UDiv:
      built = kind == .Float ? builder.buildFDiv(lhs, rhs) : builder.buildUDiv(lhs, rhs)
    case .SRem:
      built = kind == .Float ? builder.buildFRem(lhs, rhs) : builder.buildSRem(lhs, rhs)
    case .URem:
      built = kind == .Float ? builder.buildFRem(lhs, rhs) : builder.buildURem(lhs, rhs)
    case .FDiv:
      built = builder.buildFDiv(lhs, rhs)
    case .FRem:
      built = builder.buildFRem(lhs, rhs)
    case .And:
      built = builder.buildAnd(lhs, rhs)
    case .Or:
      built = builder.buildOr(lhs, rhs)
    case .Xor:
      built = builder.buildXor(lhs, rhs)
    case .Shl:
      built = builder.buildShl(lhs, rhs)
    case .Shr:
      built = kind == .Unsigned ? builder.buildLShr(lhs, rhs) : builder.buildAShr(lhs, rhs)
    case .Eq:
      built = compare(instruction, lhs: lhs, rhs: rhs, kind: kind, predicate: .EQ)
    case .Ne:
      built = compare(instruction, lhs: lhs, rhs: rhs, kind: kind, predicate: .NE)
    case .Lt:
      built = compare(instruction, lhs: lhs, rhs: rhs, kind: kind, predicate: nil)
    case .Le:
      built = compare(instruction, lhs: lhs, rhs: rhs, kind: kind, predicate: nil)
    case .Gt:
      built = compare(instruction, lhs: lhs, rhs: rhs, kind: kind, predicate: nil)
  ...
  private func emitBuiltin(_ op: TIR.ArithOp, _ instruction: TIR.Call) {
    ...
    let built: LLVMSwiftBinding.Value?
    if op.isCompare {
      built = emitBuiltinCompare(op: op, lhs: lhsValue, rhs: rhsValue, kind: kind)
    } else {
      built = emitBuiltinBinary(op: op, lhs: lhsValue, rhs: rhsValue, kind: kind)
    }
    if let built, let result = instruction.result {
      valueMap[ObjectIdentifier(result)] = built
    }
  ...
  public override func visitGlobalAddr(
    ...
    if let llvmGlobal = globalMap[value.globalId] {
      self.valueMap[ObjectIdentifier(value)] = llvmGlobal
    }
    return nil
  }
  ...
  public override func visitInlineAsm(
    ...
    unsupported("InlineAsm", at: instruction.sourceRange)
    return nil
  }
