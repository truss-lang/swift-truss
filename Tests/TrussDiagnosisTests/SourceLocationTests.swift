...
@Test func sourceLocationInequalityDifferentFile() {
    let buffer1 = StringSourceBuffer(filePath: "a.truss", content: "let x")
    let buffer2 = StringSourceBuffer(filePath: "b.truss", content: "let x")
...
@Test func sourceLocationEqualitySameFileNameDifferentBufferInstance() {
    let buffer1 = StringSourceBuffer(filePath: "a.truss", content: "let x")
    let buffer2 = StringSourceBuffer(filePath: "a.truss", content: "different")
...
@Test func sourceLocationStoresAllFields() {
    ...
    #expect(loc.buffer.filePath == "a.truss")
    #expect(loc.offset == 3)
    #expect(loc.line == 1)
    #expect(loc.column == 4)
}
